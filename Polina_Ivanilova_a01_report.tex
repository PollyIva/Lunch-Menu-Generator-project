...
% =============================================================================
\begin{abstract}
  \noindent People daily spend a lot of time solving the basic problem of deciding what to
  eat for lunch. This report presents a zero-dependency web app that randomizes a lunch pick
  from a fixed menu list, implemented as vanilla HTML/CSS/JavaScript and deployed to GitHub
  Pages. Functional verification showed that 3 of 12 attempted Font Awesome icons do not
  exist in the free 6.4.0 icon set, rendering those dishes blank; a switch to native emoji
  restored 12/12 pictures, and a final upgrade to self-hosted Twemoji SVG artwork
  (\texttt{assets/food/}, CC-BY~4.0) provides consistent, offline illustrations for every
  dish. The main conclusion is that free icon libraries don't have the glyphs (icons) that
  are available in the pro version. Therefore, before using a glyph, you need to check the
  availability of the icons in the style you want.
\end{abstract}

\keywords{menu generator, random selection, GitHub Pages, Font Awesome, Twemoji, SVG}
...
\textbf{Problem statement.} Users routinely face "what to eat for lunch?" decisions that take too much effort for a low-stakes outcome, a
classic instance of decision fatigue described in~\cite{schwartz2004paradox}. Our recommendation service should remove that problem with minimal user interaction --- one
click, one answer.

\textbf{Motivation.} This is the first assignment of an AI-assisted Recommender Systems course, whose goal is a working baseline that later weeks will extend
...
\textbf{Concrete example.} The user opens the live page (https://pollyiva.github.io/Lunch-Menu-Generator-project/), sees 12 possible dishes, and can click \textbf{``Generate Lunch!''} to get a new dish. The app shows ``Thinking\ldots'' for 500 ms and then displays a random dish with a picture, e.g.\ \textbf{Ramen}.
...
\begin{itemize}[leftmargin=2em,itemsep=1pt]
  \item \emph{Server-side backend API} --- rejected because it is unnecessary for a static page and would add extra latency and hosting costs.
  \item \emph{Font Awesome} --- initially used, but some required icons were only available in the Pro version, causing several icons to be missing.
  \item \emph{Emoji} --- used as a temporary replacement, but rejected because they did not look professional enough.
  \item \emph{Self-hosted Twemoji SVGs} --- chosen as the final solution. The icons are stored directly in the project, require no CDN, and provide a consistent visual style for all 12 dishes.
\end{itemize}
...
\begin{itemize}[leftmargin=2em,itemsep=1pt]
  \item The failure was \emph{silent}: no 404, no console error, just empty space. The free
        icon search on the Font Awesome site does not show which icons are included with the free build.
...
\textbf{Next improvement.} Stopping repetition: track the last served dish and exclude it from the next draw, and persist the daily pick in \texttt{localStorage} so a user who reopens the page keeps the choice until a new day. This is a small, testable change on top of the current random baseline.
...
\begin{itemize}[leftmargin=2em,itemsep=1pt]
  \item Manually tested the application in a browser and confirmed that random dish generation works correctly.
  \item Checked that the dish images are displayed correctly after switching to self-hosted Twemoji SVGs.
  \item Checked the published version of the project on GitHub Pages.
  \item Partially rewrote the report.
\end{itemize}
...
\url{https://fontawesome.com/search?ic=free-collection} (accessed 2026-09-14).
